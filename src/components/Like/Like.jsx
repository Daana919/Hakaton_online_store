...
export default function Like() {
  ...
  return (
    <div className="like">
      <div className="items">Favourites: {like ? like.products.length : 0}</div>
            ...
            <Card sx={{ maxWidth: 250 }} className="like-list">
              <CardActions sx={{ display: "flex", justifyContent: "center" }}>
                <DeleteOutlineIcon
                  size="small"
                  color="#a8741a"
                  onClick={() => deleteProductFromLike(row.item.id)}
                >
                  Delete
                </DeleteOutlineIcon>
                <IconButton
                  size="small"
                  onClick={() => addProductToCart(row.item)}
                >
                  <AddShoppingCartOutlinedIcon
                    style={{
                      color: `${
                        checkProductInCart(row.item.id) ? "#a8741a" : "#C8BCAC"
                      }`,
                    }}
                  />
                </IconButton>
